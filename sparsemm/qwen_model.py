...
from sparsemm.sparsemm_utils import init_snapkv, init_pyramidkv, init_adakv, init_sparsemm, init_mask,init_l2norm,init_headquant
import math
from flash_attn import flash_attn_func, flash_attn_varlen_func
from flash_attn.bert_padding import index_first_axis, pad_input, unpad_input
from sparsemm.sparsemm_utils import DynamicCacheSplitHeadFlatten
from sparsemm.sparsemm_utils import QuantizedCacheVLM
...
def qwen_flash_attn2_forward_headquant(self,
    hidden_states: torch.Tensor,
    attention_mask: Optional[torch.Tensor] = None,
    position_ids: Optional[torch.LongTensor] = None,
    past_key_value: Optional[Cache] = None,
    ...
    if is_prefill:
        alloc_bits=self.kv_cluster._allocate_bits_per_layer()[self.layer_idx]
        past_key_value.update(key_states, value_states, self.layer_idx, cache_kwargs,alloc_bits)
        # repeat k/v heads if n_kv_heads < n_heads
        key_states = repeat_kv(key_states, self.num_key_value_groups)
        value_states = repeat_kv(value_states, self.num_key_value_groups)
        dropout_rate = 0.0 if not self.training else self.attention_dropout
        ...
        alloc_bits=self.kv_cluster._allocate_bits_per_layer()[self.layer_idx]
        key_states,value_states = past_key_value.update(key_states, value_states, self.layer_idx, cache_kwargs,alloc_bits)   # (B, H, T_cached, D)
        key_states = repeat_kv(key_states, self.num_key_value_groups)
        value_states = repeat_kv(value_states, self.num_key_value_groups)
        dropout_rate = 0.0 if not self.training else self.attention_dropout
        input_dtype = query_states.dtype
        ...
        attn_output = attn_output.reshape(bsz, 1, self.hidden_size).contiguous()
...
def qwen_flash_attn2_forward_l2norm(
    self,
    hidden_states: torch.Tensor,
    attention_mask: Optional[torch.Tensor] = None,
    position_ids: Optional[torch.LongTensor] = None,
    ...
    query_states = self.q_proj(hidden_states)
    origin_key_states = self.k_proj(hidden_states)
    origin_value_states = self.v_proj(hidden_states)

    query_states = query_states.view(bsz, q_len, self.num_heads, self.head_dim).transpose(1, 2)
    origin_key_states = origin_key_states.view(bsz, q_len, self.num_key_value_heads, self.head_dim).transpose(1, 2)
    origin_value_states = origin_value_states.view(bsz, q_len, self.num_key_value_heads, self.head_dim).transpose(1, 2)

    kv_seq_len = origin_key_states.shape[-2]
    if past_key_value is not None:
        if self.layer_idx is None:
            raise ValueError(
                f"The cache structure has changed since version v4.36. If you are using {self.__class__.__name__} "
    ...
    if position_embeddings is None:
        ...
        cos, sin = self.rotary_emb(origin_value_states, position_ids)
    else:
        cos, sin = position_embeddings
        
    query_states, origin_key_states = apply_multimodal_rotary_pos_emb(
        query_states, origin_key_states, cos, sin, self.rope_scaling["mrope_section"]
    )
    ...
    if q_len == 1: # decoding
        ...
        key_states_compress, value_states_compress = self.kv_cluster.update_kv(attn_output,origin_key_states, origin_value_states)
        past_key_value.update(key_states_compress, value_states_compress, self.layer_idx, cache_kwargs)
...
def headquant_qwen_forward(
    ...
    cache_config=QuantizedCacheConfig(backend="QuantizedCacheVLM",q_group_size=32,residual_length=128,axis_key=-1,axis_value=-1)
    return_legacy_cache = False
    if (
        use_cache and not (type(past_key_values) ==QuantizedCacheVLM ) and not self.training
    ):  # kept for BC (non `Cache` `past_key_values` inputs)
        # return_legacy_cache = True  #! For 4.41 version.
        past_key_values = QuantizedCacheVLM.from_legacy_cache(past_key_values,cache_config=cache_config)
        logger.warning_once(
